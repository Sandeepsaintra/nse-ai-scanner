...
def process_trading_workstation_logic(stock_df, nifty_df, asset_name):
    ...
    # --- DIRECT DETERMINISTIC DIRECTIONAL BIAS (Guarantees Calculations Run) ---
    bias_direction = "BULLISH" if current_price > ema20 else "BEARISH"

    # --- LAYER-BASED WEIGHTED SCORING SYSTEM ---
    trend_score = 25 if (current_price > ema20 and ema20 > ema50) or (current_price < ema20 and ema20 < ema50) else 10
    momentum_score = 20 if (bias_direction == "BULLISH" and rsi > 52) or (bias_direction == "BEARISH" and rsi < 48) else 8
    ...
    regime_score = 10 if bias_direction == nifty_regime else 3
    ...
    if total_confidence >= 80: ranking_band = "HIGH CONVICTION"
    elif total_confidence >= 65: ranking_band = "STRONG SETUP"
    else: ranking_band = "TRADEABLE WATCHLIST"
    
    trend_quality_pct = int((trend_score / 25.0) * 100)
    ...
    if bias_direction == "BULLISH":
        ...
        t3 = current_price + (stop_loss_dist * 4)
    else:
        stop_loss = current_price + stop_loss_dist
        t1 = current_price - stop_loss_dist
        t2 = current_price - (stop_loss_dist * 2)
...
if batch_data is not None and nifty_raw is not None and bn_raw is not None:
    ...
    bn_last = bank_nifty_last = bn_raw['CLOSE'].iloc[-1]
    ...
    if st.button("🚀 Execute High-Conviction Core Scan", use_container_width=True):
        ...
        if processed_setups:
            ...
            for _, row in df_results.iterrows():
                card_tag = "🟢" if row['Bias'] == "BULLISH" else "🔴"
                with st.expander(f"{card_tag} **{row['Symbol']}** | Score: {row['Score']}/100 ➔ {row['Bias']} BIAS"):
                    
                    st.markdown(f"## {row['Symbol']} — {row['Bias']} POTENTIAL SETUP")
                    
                    # Component Quality Progress Metrics
    ...
    st.error("Pipeline Failure: Unable to fetch consolidated data matrix from network layers.")
